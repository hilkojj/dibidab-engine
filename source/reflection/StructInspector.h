...
namespace dibidab
{
    ...
    {
      public:
        StructInspector(const dibidab::StructInfo &structInfo);
      ...
      private:
        bool drawKeyValue(json &key, json &value, const std::string &keyType, const std::string &valueType,
            bool bEditKey, bool *bRemove = nullptr);
        ...
        bool drawInsertIntoStructure(json &structure, const std::string &keyType, const std::string &valueType,
            bool bInsertAsPair = false);
        ...
        std::map<unsigned int, StructInspector> subStructEditors;
